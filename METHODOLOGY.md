...
- ✅ **Stage 0 (normalization):** implemented, tested, all 6 train/test files cached as parquet.
- ✅ **Stage 1 (blocking):** run on the full train set. 119.7M candidate pairs for 2.2M S1 entities (~54/S1 avg, a >99.999% reduction from the ~10.3M raw S2+S3 pool). **97.4% pair recall** overall (US 98.6%, India 95.5%) — this is the ceiling the matcher below can't exceed.
- ✅ **Stage 2 (pairwise features + LightGBM):** 16 features computed for all 119.7M pairs (string similarity, address/number overlap, embedding cosine, retrieval rank/margin, generic-name frequency). Trained on a 90/10 split by S1 entity (21.9M training rows: all positives + subsampled negatives; 11.9M full validation rows, no subsampling). **Validation AUC = 0.9989, AP = 0.9868.** `cand_rank` (the embedding retriever's own ranking) is by far the strongest feature, followed by address token overlap, embedding cosine, and core-name similarity.
- ✅ **Stage 3 (decoding):** each S2/S3 record assigned to its single highest-probability S1 match (never more than one, matching the verified 1:1 structure), kept only above a tuned threshold τ. Swept τ against the actual macro F0.5 metric (not just AUC) — score climbs from 0.797 at τ=0.30 to a flat peak of **0.9359 at τ=0.91** (US 0.948, India 0.917), then falls off past τ≈0.93. Picked τ=0.91 from the middle of that flat region rather than the single best point, since the test set's true singleton rate may differ slightly from train's.
- **End-to-end validated macro F0.5 ≈ 0.936** on the held-out split — this is our current offline leaderboard number.
- ✅ **Test-set inference:** full pipeline run on the actual test set — 1,732,544 S1 entities, 120.8M candidate pairs (~69.7/S1), scored and decoded at the same τ=0.91. `output/matching_results.tsv` and `output/candidate_pairs.tsv` written and **pass the official `validate_submission.py` in both default and `--check-ids` modes.**
  - Test singleton rate: 5.89% overall (US 6.1%, India 6.2%, **France 4.2%**), mean 3.19 matches/S1 — closely matches train's 5.6% singleton rate / 3.46 mean, including for France, which the model never saw during training. This is the result we were hoping for from the "no country feature + language-agnostic normalization + multilingual embeddings" design: France isn't behaving like a degenerate edge case.
