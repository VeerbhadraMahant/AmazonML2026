...
- ⏭ Next: run the full pipeline (Stages 1-3) on the actual **test** set to produce the real `matching_results.tsv` / `candidate_pairs.tsv` submission files, validate format, and do a first leaderboard submission. Possible follow-ups if time remains: tighten India blocking recall (95.5% vs US's 98.6%), and error-analyze the confusable near-duplicate S1 names (e.g. "Primary Care Group" recurring 253x) to see if the matcher is leaning on address features enough there.
